  ...
  <div class="container">
    ...
    <div class="action-buttons-bottom">
      <button
        ...
        :class="{ 'btn-deactivate': listing.is_active, 'btn-activate': !listing.is_active }"
        class="btn-toggle-active"
      >
        {{ listing.is_active ? 'Скрыть объявление' : 'Активировать объявление' }}
      </button>

      <button @click="deleteListing(listing.id)" class="btn delete-btn">
...
import { useForm, usePage, router } from '@inertiajs/vue3'; // Добавьте 'router' сюда
...
const submitForm = () => {
  form.post(`/listings/${props.listing.id}`, { // ИЗМЕНЕНО: используем form.post с указанием _method: 'patch'
    onSuccess: () => {
      // Inertia автоматически обновит пропсы и страницу после успешного ответа
      alert('Объявление успешно обновлено!');
    },
    onError: (errors) => {
      console.error('Ошибка обновления объявления:', errors);
      alert('Пожалуйста, исправьте ошибки в форме: ' + JSON.stringify(errors));
    },
  });
};

// НОВАЯ ФУНКЦИЯ: Для полного удаления объявления
const deleteListing = (listingId) => {
    if (confirm('Вы уверены, что хотите полностью удалить это объявление? Это действие необратимо.')) {
        router.delete(`/listings/${listingId}`, {
            onSuccess: () => {
                alert('Объявление успешно удалено.');
                router.visit('/my-listings'); // Перенаправляем на дашборд после удаления
            },
            onError: (errors) => {
                console.error('Ошибка при удалении объявления:', errors);
                alert('Ошибка при удалении объявления: ' + (errors.message || JSON.stringify(errors)));
            },
        });
    }
};

// НОВАЯ ФУНКЦИЯ: Для переключения статуса активности объявления
const toggleActiveStatus = () => {
    const action = props.listing.is_active ? 'скрыть' : 'активировать';
    if (confirm(`Вы уверены, что хотите ${action} это объявление?`)) {
        router.patch(`/listings/${props.listing.id}/toggle-active`, {}, {
            onSuccess: () => {
                // Inertia автоматически обновит пропсы (включая listing.is_active)
                // после успешного ответа от сервера
                alert(`Объявление успешно ${action === 'скрыть' ? 'скрыто' : 'активировано'}.`);
            },
            onError: (errors) => {
                console.error('Ошибка при переключении статуса:', errors);
                alert('Ошибка при переключении статуса: ' + (errors.message || JSON.stringify(errors)));
            }
        });
    }
};
...
.action-buttons-bottom {
    ...
    margin-top: 30px;
    padding-top: 20px;
    border-top: 1px solid #eee; /* Разделитель, если нужен */
}
...
.btn-deactivate {
    background-color: #ffc100; /* Желтый для "скрыть" */
    color: #333;
}
...
.btn-activate {
    background-color: #28a745; /* Зеленый для "активировать" */
    color: white;
}
.btn-activate:hover {
...
.delete-btn {
    background-color: #dc3545; /* Красный для полного удаления */
    color: white;
}
.delete-btn:hover {
